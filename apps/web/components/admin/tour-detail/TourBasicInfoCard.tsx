...
import type { LocationOption, TourDetailForm, TransportOption } from "@/lib/admin/tourDetail";
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            onChange={(event) =>
              onTourChange((prev) => ({ ...prev, tour_type: event.target.value }))
            }
          >
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            onChange={(event) =>
              onTourChange((prev) => ({ ...prev, transport_id: Number(event.target.value) }))
            }
          >
      ...
      <div className="space-y-4 border-t border-slate-50 pt-4">
        ...
        <div className="custom-scrollbar grid max-h-[300px] grid-cols-2 gap-3 overflow-y-auto rounded-3xl border border-slate-100 bg-slate-50 p-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4">
          {locations
            .filter((location) => {
            ...
            .map((location) => {
              const isSelected = tour.destinations?.includes(location.location_id);
                    ...
                    onChange={(event) => {
                      ...
                      onTourChange((prev) => ({
                        ...
                        destinations: event.target.checked
                          ...
                          : currentDestinations.filter((id) => id !== location.location_id),
                      }));
                    }}
                  />
                  <span
                    className={`text-[11px] font-medium transition-colors ${
                      isSelected ? "font-extrabold text-blue-600" : "text-slate-600"
                    }`}
                  >
                    {location.name}
                  </span>
      ...
      <div className="space-y-2 border-t border-slate-50 pt-4">
        ...
        <textarea
          ...
          onChange={(event) =>
            onTourChange((prev) => ({ ...prev, description: event.target.value }))
          }
        />
      </div>
    </AdminFormCard>
