        ...
        <table className="w-full border-collapse text-left">
          ...
          <tbody className="divide-y divide-slate-100 text-sm">
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-20 text-center">
                  <p className="font-medium italic text-slate-400">Không tìm thấy người dùng phù hợp</p>
                </td>
              </tr>
            ) : (
              users.map((user) => (
                <tr key={user.user_id} className="group transition-colors hover:bg-slate-50/50">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                      <button
                        ...
                        title={user.accounts?.status === 1 ? "Khóa tài khoản" : "Mở khóa tài khoản"}
                      ...
                      >
                        {user.accounts?.status === 1 ? <Lock size={16} /> : <Unlock size={16} />}
                      </button>
                      <button
                        onClick={() => onDelete(user.user_id)}
                        className="rounded-lg bg-slate-50 p-2 text-slate-400 transition-colors hover:bg-rose-50 hover:text-rose-600"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
