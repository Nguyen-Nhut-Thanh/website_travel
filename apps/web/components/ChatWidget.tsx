...
const AI_BASE_URL = process.env.NEXT_PUBLIC_AI_BASE || "http://localhost:8000";
              ...
              setMessages([
                {
                  role: "assistant",
                  content:
                    "Xin chào! Tôi là Trợ lý ảo Travol. Tôi có thể giúp bạn tìm kiếm những hành trình tuyệt vời nhất. Bạn muốn đi đâu hôm nay?",
                },
              ]);
            }
          })
            ...
            setMessages([
              {
                role: "assistant",
                content:
                  "Chào bạn! Chatbot Travol đã sẵn sàng. Hãy cho tôi biết kế hoạch của bạn nhé!",
              },
            ]);
          });
      }
        ...
        socket.emit(
          ...
          (res: any) => {
            if (res && res.conversation_id) {
              ...
              if (!convId) {
                fetch(`${API_BASE}/chat/messages/${res.conversation_id}`, {
                  headers: { Authorization: `Bearer ${currentToken}` },
                })
                  .then((r) => r.json())
                  .then((data) => {
      ...
      try {
        ...
        if (!finalConvId) {
          const res: any = await new Promise((resolve) => {
            socket.emit("chat:join", {}, (r: any) => resolve(r));
          });
          if (res && res.conversation_id) {
            setConvId(res.conversation_id);
                        ...
                        {msg.recommended_tours.map((tour) => (
                          <Link key={tour.tour_id} href={`/tours/${tour.tour_id}`} className="flex gap-3 bg-white p-2 rounded-2xl border border-slate-100 hover:border-blue-200 transition-all">
                            <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0">
                              <img src={tour.image_url || "/next.svg"} alt={tour.name} className="w-full h-full object-cover" />
                            </div>
