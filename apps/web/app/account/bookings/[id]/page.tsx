...
export default function BookingDetailPage() {
  ...
  const summary = useMemo(() => {
    ...
    const infantTotal =
      Number(booking.infant_unit_price || 0) *
      Number(booking.infant_count || 0);
  ...
  const visibleCustomerNote = getVisibleCustomerNote(booking.note);
  const departureName =
        ...
        <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.06)]">
          ...
          >
            <div className="grid gap-4 md:grid-cols-2">
              <LabelValue
                label="Họ và tên"
                value={booking.contact_name || "Đang cập nhật"}
                strong
              />
              <LabelValue
                label="Số điện thoại"
                value={booking.contact_phone || "Đang cập nhật"}
                strong
              />
              <div className="md:col-span-2">
                <LabelValue
                  label="Email"
                  value={booking.contact_email || "Đang cập nhật"}
                />
              </div>
              {visibleCustomerNote ? (
                <div className="md:col-span-2">
                  <LabelValue label="Ghi chú" value={visibleCustomerNote} />
                </div>
              ) : null}
            </div>
          </Section>
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="border-t border-slate-100 pt-3">
                      ...
                      <div className="mt-3 flex items-center justify-between text-slate-500">
                        <span>Giao dịch</span>
                        <span>
                          {summary.payment?.transaction_code || "Đang cập nhật"}
                        </span>
                      </div>
                      <div className="mt-3 flex items-center justify-between text-slate-500">
                    ...
                    {canRequestCancel ? (
                      <button
                        ...
                        className="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-4 text-sm font-semibold text-rose-700 transition hover:bg-rose-100 disabled:opacity-70"
                      >
                        {requestingCancel ? (
                          <Loader2 className="h-4 w-4 animate-spin" />
                        ) : (
        ...
        <AdminModalShell
          ...
          footer={
            <>
              ...
              >
                {requestingCancel ? "Đang gửi..." : "Xác nhận gửi yêu cầu"}
              </button>
            </>
          }
        >
          <div className="space-y-5 p-6">
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
              Phí hủy tour và số tiền hoàn sẽ được thông báo sau, căn cứ theo
              chính sách chuyển/hủy tour của chương trình này.
            </div>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows={5}
                placeholder="Nhập lý do để admin tiện xử lý booking này."
