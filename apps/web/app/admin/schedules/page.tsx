...
export default function AdminSchedulesPage() {
  ...
  const handleDeleteSchedule = async (scheduleId: number) => {
    if (!confirmAction("Bạn có chắc muốn xóa đợt khởi hành này?")) return;

    try {
      const response = await adminFetch(`/admin/tours/schedules/${scheduleId}`, { method: "DELETE" });
      if (response.ok) {
        success("Đã xóa lịch khởi hành thành công");
        setSchedules((current) => current.filter((schedule) => schedule.tour_schedule_id !== scheduleId));
      }
    } catch {
  ...
  const handleToggleStatus = async (scheduleId: number, currentStatus: number, bookedCount: number) => {
      ...
      if (response.ok) {
        ...
        setSchedules((current) =>
          current.map((schedule) =>
            schedule.tour_schedule_id === scheduleId ? { ...schedule, status: nextStatus } : schedule,
          ),
        );
      }
    } catch {
  ...
  const filteredTours = tours.filter(
    (tour) => matchesTourSearch(tour, searchTerm) && matchesTourStatusFilter(tour, tourStatusFilter),
  );
  ...
  const paginatedTours = filteredTours.slice((tourPage - 1) * pageSize, tourPage * pageSize);
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-6">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          <h1 className="text-2xl font-bold leading-tight text-slate-900">Quản lý lịch khởi hành</h1>
