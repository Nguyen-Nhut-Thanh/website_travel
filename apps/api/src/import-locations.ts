...
const REGION_MAPPING = {
  north: {
    name: "Miền Bắc",
    slug: "mien-bac",
    codes: ["01", "02", "04", "06", "08", "10", "11", "12", "14", "15", "17", "19", "20", "22", "24", "25", "26", "27", "30", "31", "33", "34", "35", "36", "37"]
  },
  central: {
    name: "Miền Trung",
    slug: "mien-trung",
    codes: ["38", "40", "42", "44", "45", "46", "48", "49", "51", "52", "54", "56", "58", "60", "62", "64", "66", "67", "68"]
  },
  south: {
    name: "Miền Nam",
    slug: "mien-nam",
    codes: ["70", "72", "74", "75", "77", "79", "80", "82", "83", "84", "86", "87", "89", "91", "92", "93", "94", "95", "96"]
  }
};
...
async function main() {
  ...
  try {
    ...
    for (const p of apiData) {
      let regionId = null;
      if (REGION_MAPPING.north.codes.includes(p.code)) regionId = regions.north.location_id;
      else if (REGION_MAPPING.central.codes.includes(p.code)) regionId = regions.central.location_id;
      else if (REGION_MAPPING.south.codes.includes(p.code)) regionId = regions.south.location_id;

      if (!regionId) continue;

      const normalizedName = normalizeProvinceName(p.name);
      const slug = p.slug || normalizedName.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[đĐ]/g, "d").replace(/[^a-z0-9]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
