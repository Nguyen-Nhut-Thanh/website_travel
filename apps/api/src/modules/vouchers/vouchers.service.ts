...
export class VouchersService {
  ...
  async validateVoucher(code: string, orderAmount: number) {
    ...
    if (!voucher) throw new BadRequestException('Mã giảm giá không hợp lệ hoặc đã hết hạn');

    const now = new Date();
    if (now < voucher.start_date) throw new BadRequestException('Mã giảm giá chưa đến thời gian sử dụng');
    if (now > voucher.expiry_date) throw new BadRequestException('Mã giảm giá đã hết hạn');
    ...
    if (orderAmount < Number(voucher.min_order_value)) {
      throw new BadRequestException(`Đơn hàng tối thiểu ${Number(voucher.min_order_value).toLocaleString()}đ để sử dụng mã này`);
    }
