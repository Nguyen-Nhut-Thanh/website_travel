import { Controller, Get, Post, Patch, Delete, Body, Param, Query, UseGuards, UseInterceptors, UploadedFile, BadRequestException, InternalServerErrorException } from '@nestjs/common';
...
export class LocationsController {
  ...
  @Get('admin/locations')
  async findAll(@Query('search') search?: string, @Query('level_id') level_id?: string) {
    return this.locationsAdminService.findAll({ search, level_id });
  }

  @UseGuards(JwtAuthGuard)
  @Get('admin/locations/by-level')
  async listByLevel(@Query('level_id') level_id: string, @Query('parent_id') parent_id?: string) {
    return this.locationsAdminService.listByLevel(Number(level_id), parent_id ? Number(parent_id) : undefined);
  }
  ...
  async uploadLocationImage(@UploadedFile() file: Express.Multer.File) {
    ...
    try {
      ...
      throw new InternalServerErrorException('Lỗi upload ảnh địa điểm: ' + error.message);
    }
  }
}
