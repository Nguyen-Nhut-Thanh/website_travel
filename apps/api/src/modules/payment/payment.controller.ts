...
export class PaymentController {
  ...
  async createPaymentUrl(
    @Body() body: { bookingId: number; amount: number },
    @Req() req: any,
  ) {
  ...
  async vnpayReturn(@Query() query: any, @Res() res: any) {
    ...
    if (result.RspCode === '00' && result.Message === 'Confirm Success') {
      return res.redirect(
        `${frontendUrl}/booking/result?status=success&orderId=${query.vnp_TxnRef.split('_')[0]}`,
      );
    } else {
      return res.redirect(
        `${frontendUrl}/booking/result?status=failed&orderId=${query.vnp_TxnRef.split('_')[0]}`,
      );
    }
  }
}
