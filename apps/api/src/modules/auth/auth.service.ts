...
import { RegisterDto, LoginDto, VerifyEmailDto, GoogleLoginDto } from './dto/auth.dto';
...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    const existing = await this.prisma.accounts.findUnique({ where: { email: dto.email } });
    if (existing) throw new BadRequestException('Email đã tồn tại trên hệ thống');
    ...
    return { message: 'Đăng ký thành công. Vui lòng kiểm tra email để nhận mã xác thực.' };
  }
  ...
  async resendVerificationCode(email: string) {
    ...
    if (account.email_verified) throw new BadRequestException('Email đã được xác thực trước đó');
    ...
    try {
      ...
      if (error.code === 'P2002') {
        ...
        if (target.includes('number_id')) {
          throw new BadRequestException('Số CCCD/Hộ chiếu này đã được sử dụng bởi người dùng khác');
        }
        throw new BadRequestException('Thông tin bị trùng lặp với người dùng khác');
      }
      throw error;
    }
  }
  ...
  async changePassword(userId: number, dto: any) {
    ...
    if (account.password_hash) {
      ...
      const isMatch = await bcrypt.compare(dto.oldPassword, account.password_hash);
    ...
    return { message: account.password_hash ? 'Đổi mật khẩu thành công' : 'Thiết lập mật khẩu thành công' };
  }

  async verifyEmail(dto: VerifyEmailDto) {
    const account = await this.prisma.accounts.findUnique({ where: { email: dto.email } });
    if (!account) throw new BadRequestException('Không tìm thấy tài khoản');
    if (account.email_verified) throw new BadRequestException('Email đã được xác thực trước đó');
    ...
    if (!account.verification_expires_at || new Date() > account.verification_expires_at) {
      throw new BadRequestException('Mã xác thực đã hết hạn hoặc không hợp lệ. Vui lòng yêu cầu mã mới.');
    }
  ...
  async login(dto: LoginDto) {
    ...
    if (!account.email_verified) {
      throw new UnauthorizedException('Email chưa được xác thực. Vui lòng xác thực trước khi đăng nhập.');
    }
  ...
  async adminLogin(email: string, password: string) {
    ...
    if (!account || !account.password_hash) throw new UnauthorizedException('Thông tin không chính xác');

    const ok = await bcrypt.compare(password, account.password_hash);
    if (!ok) throw new UnauthorizedException('Thông tin không chính xác');

    if (!account.users?.is_staff) throw new UnauthorizedException('Bạn không có quyền truy cập trang quản trị');
